...
public final class EudiWallet: ObservableObject {
  ...
  @discardableResult public func loadVPDocuments() async throws -> [WalletStorage.PresentationLog]? {
	...
	public func prepareServiceDataParameters(docType: String? = nil, dataFormat: DataFormat = .cbor ) throws -> [String : Any] {
		print("debug: WalletKit EudiWallet: prepareServiceDataParameters")
		var parameters: [String: Any]
		switch dataFormat {
		case .cbor:
			guard var docs = try storageService.loadDocuments(), docs.count > 0 else {
				print("debug: WalletKit EudiWallet: prepareServiceDataParameters: storageService.loadDocuments: No documents found")
			...
			if let docType { guard docs.count > 0 else {
				print("debug: WalletKit EudiWallet: prepareServiceDataParameters: docType: No documents of type \(docType) found")
			...
			guard cborsWithKeys.count > 0 else {
				print("debug: WalletKit EudiWallet: prepareServiceDataParameters: cborsWithKeys.count: Documents decode error")
	...
	public func resolveRequestDocType(flow: FlowType) async -> String? {
		print("debug: WalletKit EudiWallet: resolveRequestDocType")
		do {
			switch flow {
			case .openid4vp(let qrCode):
				...
				print("debug: WalletKit EudiWallet: resolveRequestDocType: success create OpenId4VpService instance")
				var result = try await openIdSvc.getResolvedRequestData()
				print("debug: WalletKit EudiWallet: resolveRequestDocType: success openIdSvc.getResolvedRequestData")
				return result
			default:
				return nil
			}
		} catch {
			print("debug: WalletKit EudiWallet: resolveRequestDocType: error")
			print(error)
			return nil
		}
	}
	...
	public func beginPresentation(flow: FlowType, docType: String? = nil, dataFormat: DataFormat = .cbor) -> PresentationSession {
		print("debug: WalletKit EudiWallet: beginPresentation")
		do {
			...
			case .openid4vp(let qrCode):
				...
				print("debug: WalletKit EudiWallet: beginPresentation: success")
